...
def build_model(model_name, num_classes=10, deploy=False, pretrained_path=None, use_pretrained=False):
    """
    构建RepVGG模型（支持加载ImageNet预训练权重）。
    :param model_name: 如 'RepVGG-A0'
    :param num_classes: 分类类别数
    :param deploy: deploy模式
    :param pretrained_path: 预训练权重路径
    :param use_pretrained: 是否使用预训练
    :return: RepVGG模型
    """
    func = get_RepVGG_func_by_name(model_name)
    model = func(deploy=deploy)

    if use_pretrained and pretrained_path and os.path.isfile(pretrained_path):
        state_dict = torch.load(pretrained_path, map_location='cpu')
        if 'state_dict' in state_dict:  # 适配可能的key
            state_dict = state_dict['state_dict']
        # 过滤不加载分类头的权重
        new_state_dict = {k: v for k, v in state_dict.items() if not k.startswith('linear')}
...
# def build_model(num_classes=10, deploy=False):
#     """
#     构建RepVGG模型。
#     :param num_classes: 分类类别数
#     :param deploy: 是否为推理部署模式（True 则为单卷积）
#     :return: RepVGG模型
#     """
#     model = create_RepVGG_A0(deploy=deploy)  # 选A0，适合小任务
#     model.linear = torch.nn.Linear(model.linear.in_features, num_classes)  # 替换分类头
#     return model